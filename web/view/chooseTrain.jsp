...
<div class="container">
    ...
    <h5>
        <a href="orderForm" style="background-color:#4390ff;border:1px solid #4390ff;border-radius:3px;color:#ffffff;display:inline-block;font-family:sans-serif;font-size:16px;line-height:44px;text-align:center;text-decoration:none;width:150px;-webkit-text-size-adjust:none;mso-hide:all;"> &larr; <fmt:message key="proceed.login" /></a>
    </h5>
    <h3><c:if test="${noFlightError == null}"><fmt:message key="choose.train"/></c:if></h3>
    <div class="row col-md-6">
        <h4><c:if test="${noFlightError != null}"><fmt:message key="nothing.found"/></c:if></h4>

        <table class="table table-striped table-bordered table-sm" >
            <tr>
                <th style="width: 10px ; background-color:dodgerblue"><span style="color: white; "><fmt:message key="train.code" /></span></th>
                <th style="width: 10px ; background-color:dodgerblue"><span style="color: white; "><fmt:message key="train.name" /></span></th>
                <th style="width: 10px ; background-color:dodgerblue"><span style="color: white; "><fmt:message key="train.date" /></span></th>
                <th style="width: 10px ; background-color:dodgerblue"><span style="color: white; "><fmt:message key="train.free" /></span></th>
            </tr>

            <c:forEach items="${trains}" var="train">
                <tr>
                    <td><span style="color: black; ">${train.getCodeTrain()}</span></td>
                    <td><span style="color: black; ">${train.getName()}</span></td>
                    <td><span style="color: black; ">${train.getDate()}</span></td>
                    <td><span style="color: black; ">${train.getFreeSeatsAmount()}</span></td>
                </tr>
            </c:forEach>
        </table>
    </div>
        ...
        <ul class="pagination">
            <c:if test="${page != 1}">
                <li class="page-item"><a class="page-link"
                                         href="chooseTrain?lang=${lang}&page=${page-1}"><fmt:message key="prev" /></a>
                </li>
            </c:if>
            ...
            <c:forEach begin="1" end="${noOfPages}" var="i">
                <c:choose>
                    ...
                    <c:otherwise>
                        <li class="page-item"><a class="page-link"
                                                 href="chooseTrain?lang=${lang}&page=${i}">${i}</a>
                        </li>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:if test="${page lt noOfPages}">
                <li class="page-item"><a class="page-link"
                                         href="chooseTrain?lang=${lang}&page=${page+1}"><fmt:message key="next" /></a>
                </li>
            </c:if>
        </ul>
    </nav>

    <h4><c:if test="${trainCodeError != null}"><fmt:message key="confirm.error"/></c:if></h4>

    <form action="confirmOrder">
        <label for="trainCode"><fmt:message key="choose.train.write" /></label>
        <input type="text" id="trainCode" name="trainCode"><br><br>
        <input type="submit" value="<fmt:message key="confirm.order" />">
    </form>
