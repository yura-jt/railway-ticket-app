...
<div class="container">
    ...
    <header class="clearfix">
        ...
        <h1><fmt:message key="admin.bills.title" /></h1>
    ...
    <div class="row col-md-6">
        ...
        <table class="table table-striped table-bordered table-sm" >
            <tr>
                <th style="width: 10px ; background-color:dodgerblue"><span style="color: white; "><fmt:message key="bill.id" /></span></th>
                <th style="width: 10px ; background-color:dodgerblue"><span style="color: white; "><fmt:message key="bill.status" /></span></th>
                <th style="width: 10px ; background-color:dodgerblue"><span style="color: white; "><fmt:message key="bill.price" /></span></th>
                <th style="width: 10px ; background-color:dodgerblue"><span style="color: white; "><fmt:message key="bill.created" /></span></th>
            </tr>

            <c:forEach items="${bills}" var="bill">
                <tr>
                    <td><span style="color: black; ">${bill.getOrderId()}</span></td>
                    <td><span style="color: black; ">${bill.getStatus()}</span></td>
                    <td><span style="color: black; ">${bill.getPrice()}</span></td>
                    <td><span style="color: black; ">${bill.getCreatedOn()}</span></td>
                </tr>
            </c:forEach>
        </table>
    </div>
        ...
        <ul class="pagination">
            <c:if test="${page != 1}">
                <li class="page-item"><a class="page-link"
                                         href="adminBills?lang=${lang}&page=${page-1}"><fmt:message key="prev" /></a>
                </li>
            </c:if>
            ...
            <c:forEach begin="1" end="${noOfPages}" var="i">
                <c:choose>
                    ...
                    <c:otherwise>
                        <li class="page-item"><a class="page-link"
                                                 href="adminBills?lang=${lang}&page=${i}">${i}</a>
                        </li>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:if test="${page lt noOfPages}">
                <li class="page-item"><a class="page-link"
                                         href="adminBills?lang=${lang}&page=${page+1}"><fmt:message key="next" /></a>
                </li>
            </c:if>
        </ul>
    </nav>
